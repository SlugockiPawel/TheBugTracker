...
//Get project priority Ids
            ...
            int priorityMedium = context.ProjectPriorities
                .FirstOrDefault(p => p.Name == ProjectPriorities.Medium.ToString()).Id;
            int priorityHigh = context.ProjectPriorities
                .FirstOrDefault(p => p.Name == ProjectPriorities.High.ToString()).Id;
            int priorityUrgent = context.ProjectPriorities
                .FirstOrDefault(p => p.Name == ProjectPriorities.Urgent.ToString()).Id;
        ...
        private static async Task SeedDefautTicketsAsync(ApplicationDbContext context)
        {
            ...
            //Get ticket type Ids
            int typeNewDev = context.TicketTypes.FirstOrDefault(p => p.Name == TicketTypes.NewDevelopment.ToString())
                .Id;
            int typeWorkTask = context.TicketTypes.FirstOrDefault(p => p.Name == TicketTypes.WorkTask.ToString()).Id;
            int typeDefect = context.TicketTypes.FirstOrDefault(p => p.Name == TicketTypes.Defect.ToString()).Id;
            int typeEnhancement = context.TicketTypes.FirstOrDefault(p => p.Name == TicketTypes.Enhancement.ToString())
                .Id;
            int typeChangeRequest = context.TicketTypes
                .FirstOrDefault(p => p.Name == TicketTypes.ChangeRequest.ToString()).Id;

//Get ticket priority Ids
            int priorityLow = context.TicketPriorities.FirstOrDefault(p => p.Name == TicketPriorities.Low.ToString())
                .Id;
            int priorityMedium = context.TicketPriorities
                .FirstOrDefault(p => p.Name == TicketPriorities.Medium.ToString()).Id;
            int priorityHigh = context.TicketPriorities.FirstOrDefault(p => p.Name == TicketPriorities.High.ToString())
                .Id;
            int priorityUrgent = context.TicketPriorities
                .FirstOrDefault(p => p.Name == TicketPriorities.Urgent.ToString()).Id;

//Get ticket status Ids
            int statusNew = context.TicketStatuses.FirstOrDefault(p => p.Name == TicketStatuses.New.ToString()).Id;
            int statusDev = context.TicketStatuses.FirstOrDefault(p => p.Name == TicketStatuses.Development.ToString())
                .Id;
            int statusTest = context.TicketStatuses.FirstOrDefault(p => p.Name == TicketStatuses.Testing.ToString()).Id;
            int statusResolved = context.TicketStatuses
                .FirstOrDefault(p => p.Name == TicketStatuses.Resolved.ToString()).Id;
            try
            {
